...
class Stone(Dataset):
    ...
    def __init__(self, root, resize, mode):
        ...
        if mode=='train': # 60%
            ...
            self.labels = self.labels[:int(0.75*len(self.labels))]
        elif mode=='val': # 20% = 60%->80%
            self.images = self.images[int(0.75*len(self.images)):]
            self.labels = self.labels[int(0.75*len(self.labels)):]
    ...
    def load_csv(self, filename):
        ...
        if not os.path.exists(os.path.join(self.root, filename)):
            ...
            with open(os.path.join(self.root, filename), mode='w', newline='') as f:
                writer = csv.writer(f)
                for img in images: # 'Stone\\bulbasaur\\00000000.png'
                    name = img.split(os.sep)[-2]
                    label = self.name2label[name]
                    # 'Stone\\bulbasaur\\00000000.png', 0
...
def main():
    ...
    #                 transforms.ToTensor(),
    # ])
    # db = torchvision.datasets.ImageFolder(root='Stone', transform=tf)
